...
public class BuyCar implements Operation {
    public void operation(Database database, Scanner sc, User user){
        ...
        try{
            ...
            ResultSet rs2 = database.getStatement().executeQuery("Select * from buys where car_id ='"+carID+"'; ");
            rs2.next();
            int user_id = rs2.getInt("user_id");
            ResultSet rs3 = database.getStatement().executeQuery("Select * from user where id ='"+user_id+"'; ");
            rs3.next();
